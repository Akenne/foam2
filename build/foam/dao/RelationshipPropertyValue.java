// WARNING: GENERATED CODE, DO NOT MODIFY BY HAND!
package foam.dao;


public class RelationshipPropertyValue extends foam.core.AbstractFObject {
  private static final foam.core.ClassInfo classInfo_ = new foam.core.ClassInfoImpl()
    .setId("foam.dao.RelationshipPropertyValue").setObjClass(foam.dao.RelationshipPropertyValue.class);
  public foam.core.ClassInfo getClassInfo() {
    return classInfo_;
  }
  public static foam.core.ClassInfo getOwnClassInfo() {
    return classInfo_;
  }
}